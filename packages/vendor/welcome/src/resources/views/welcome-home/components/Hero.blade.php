...
/* Barre de recherche : composant FIXE du hero.
   Le header étant en position:fixed et superposé au hero, on positionne la barre
   JUSTE SOUS le header (responsive : on mesure la hauteur réelle du header).
   Elle reste dans le hero (position:absolute) et défile avec lui — elle ne
   « flotte » PLUS au-dessus des sections après le hero. */
document.addEventListener('DOMContentLoaded', function () {
    var layer = document.querySelector('.go-home-hero .go-hero-search-layer');
    var header = document.querySelector('.header-v2');
    if (!layer || !header) return;
    function positionSearch() {
        var hb = header.getBoundingClientRect().height || 0;
        layer.style.top = (hb + 10) + 'px';
    }
    positionSearch();
    window.addEventListener('resize', positionSearch, { passive: true });
    window.addEventListener('load', positionSearch);
});
</script>
