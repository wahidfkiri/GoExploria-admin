...
<style>
    ...
    .stat-item {
        ...
        transition: var(--transition); min-width: 200px; flex: 1;
    }
    ...
    .sidebar-toggle {
        position: absolute; top: 20px; right: 370px; z-index: 1001;
    ...
    .place-category {
        ...
        color: white; border-radius: 20px; font-size: 0.8rem; font-weight: 500; margin-bottom: 10px;
    }
    ...
    .form-select {
        ...
        border-radius: 8px; font-size: 0.9rem; transition: var(--transition);
    }
    ...
    .no-results i { font-size: 3rem; color: #ddd; margin-bottom: 20px; }
    ...
    .modal-content {
        ...
        margin: 50px auto; width: 90%; max-width: 900px;
    ...
    .toast-notification {
        ...
        box-shadow: var(--shadow-hover); border-left: 4px solid var(--primary);
        z-index: 2002; animation: slideInRight 0.3s ease; max-width: 350px;
    }
    ...
    .youtube-video-container {
        ...
        overflow: hidden; border-radius: 8px; margin: 8px 0; background: #000;
    }
    ...
    .ops-slide-meta-icon i { color: white; font-size: 13px; }
    .ops-slide-meta-info { flex: 1; min-width: 0; }
    .ops-slide-meta-name {
        font-weight: 600; font-size: 13px; color: #1a1a1a;
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    .ops-slide-meta-loc { font-size: 11px; color: #888; margin-top: 1px; }
    .ops-slide-meta-btn {
        flex-shrink: 0;
        padding: 6px 14px;
    ...
    .ops-slide-meta-btn:hover { background: var(--primary-dark); }
    .ops-controls {
        display: flex;
        align-items: center;
    ...
    .ops-dots-wrap { display: flex; align-items: center; gap: 8px; }
    ...
    .ops-dot.active { background: var(--primary); transform: scale(1.25); }
    .ops-counter { font-size: 12px; color: #999; font-weight: 500; min-width: 40px; text-align: center; }
    ...
    @media (max-width: 768px) {
        ...
        .sidebar-right { width: 100%; transform: translateX(100%); }
        .sidebar-right.active { transform: translateX(0); }
        .sidebar-toggle { right: 20px; top: 20px; }
        .stat-item { min-width: 150px; padding: 20px; }
        .stat-number { font-size: 2.5rem; }
    }
    @media (max-width: 576px) {
        .info-card { padding: 25px; }
        .info-title { font-size: 1.5rem; }
        .app-container { height: 350px; }
...
class InteractiveMap {
    ...
    loadFiltersStatic() {
        ...
        if (provinceFilter) {
            ...
            provinces.forEach(prov => {
                ...
                opt.dataset.lat = prov.lat;
                opt.dataset.lng = prov.lng;
    ...
    updateProvincesCount(count)  { if (document.getElementById('totalProvinces'))  this.animateCounter('totalProvinces',  0, count); }
    updateCategoriesCount(count) { if (document.getElementById('totalCategories')) this.animateCounter('totalCategories', 0, count); }
    ...
    zoomToProvince(provinceCode) {
        ...
        const province = this.getStaticProvinces().find(p => p.code === provinceCode);
        if (province) {
    ...
    clearMarkers() {
        Object.values(this.markers).forEach(({ marker }) => { if (marker && marker.remove) marker.remove(); });
        this.markers = {};
    }
    ...
    createMarker(place) {
        const icon = L.divIcon({
            ...
            html: `<div class="marker-icon" style="background:${this.getCategoryColor(place.category)};">
        ...
        marker.on('mouseover', () => {
            ...
            hoverTimeout = setTimeout(() => {
                ...
                const el = document.querySelector(`.place-item[data-id="${place.id}"]`);
                if (el) el.classList.add('active');
                const ico = marker.getElement()?.querySelector('.marker-icon');
                if (ico) ico.classList.add('highlighted');
            }, 150);
        });
        ...
        marker.on('mouseout', () => {
            clearTimeout(hoverTimeout);
            hoverTimeout = setTimeout(() => {
                const popEl = document.querySelector('.leaflet-popup');
                if (!popEl || !popEl.matches(':hover')) {
                    this.map.closePopup();
                    const el = document.querySelector(`.place-item[data-id="${place.id}"]`);
                    if (el) el.classList.remove('active');
                    const ico = marker.getElement()?.querySelector('.marker-icon');
                    if (ico) ico.classList.remove('highlighted');
                }
            }, 200);
        });
        ...
        marker.on('click', (e) => {
            ...
            if (!this.map.hasLayer(popup)) popup.setLatLng(marker.getLatLng()).openOn(this.map);
        });
    ...
    createPopupContent(place) {
        ...
        if (youtubeId && youtubeId.includes('?')) youtubeId = youtubeId.split('?')[0];
        ...
        const videoHtml = youtubeId ? `
            <div class="youtube-video-container" style="position:relative;width:100%;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:8px;margin:8px 0;background:#000;">
                <iframe src="https://www.youtube.com/embed/${youtubeId}?autoplay=0&mute=1&controls=1&modestbranding=1&rel=0&showinfo=0"
            ...
            <div class="hover-popup-content" data-place-id="${place.id}">
                <div style="padding:12px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
                    ...
                    <button class="popup-details-btn"
                        onclick="event.stopPropagation();window.mapApp.showPlaceModal(${JSON.stringify(place).replace(/"/g, '&quot;')})">
    ...
    createOtherPlacesVideoSlider(currentPlace) {
        ...
        const slides = others.map((p, i) => {
            let vid = p.youtube_id;
            if (vid && vid.includes('?')) vid = vid.split('?')[0];
            const placeJson = JSON.stringify(p).replace(/"/g, '&quot;');
            return `
                <div class="ops-slide" data-index="${i}" style="display:${i === 0 ? 'block' : 'none'};">
                    <div class="ops-slide-video">
                        <iframe
                    ...
                    <div class="ops-slide-meta">
                        <div class="ops-slide-meta-icon" style="background:${this.getCategoryColor(p.category)};">
                            <i class="${this.getCategoryIcon(p.category)}"></i>
                        </div>
                        <div class="ops-slide-meta-info">
                            <div class="ops-slide-meta-name">${this.escapeHtml(p.name)}</div>
                            <div class="ops-slide-meta-loc"><i class="fas fa-map-marker-alt"></i> ${p.province || 'Canada'}</div>
                        </div>
                        <button class="ops-slide-meta-btn"
                            onclick="window.mapApp.closeModal();setTimeout(()=>window.mapApp.showPlaceModal(${placeJson}),200)">
                            Voir lieu
                        </button>
                    </div>
                </div>`;
        ...
        /* dots */
        const dotsHtml = others.map((_, i) => `
            <span class="ops-dot ${i === 0 ? 'active' : ''}"
                  onclick="(function(){
                      var sl=document.getElementById('${sliderId}');
                      var items=sl.querySelectorAll('.ops-slide');
                      var cur=Array.from(items).findIndex(s=>s.style.display==='block');
                      items[cur].style.display='none';
                      items[${i}].style.display='block';
                      sl.querySelectorAll('.ops-dot').forEach((d,idx)=>d.classList.toggle('active',idx===${i}));
                      sl.querySelector('.ops-counter').textContent='${i+1} / ${others.length}';
                  })()">
            </span>`).join('');

        const controlsHtml = others.length > 1 ? `
            <div class="ops-controls">
                <button class="ops-nav-btn"
                    onclick="(function(){
                        var sl=document.getElementById('${sliderId}');
                        var items=sl.querySelectorAll('.ops-slide');
                        var cur=Array.from(items).findIndex(s=>s.style.display==='block');
                        items[cur].style.display='none';
                        var prev=(cur-1+items.length)%items.length;
                        items[prev].style.display='block';
                        sl.querySelectorAll('.ops-dot').forEach((d,i)=>d.classList.toggle('active',i===prev));
                        sl.querySelector('.ops-counter').textContent=(prev+1)+' / '+items.length;
                    })()">&#8249;</button>
                <div class="ops-dots-wrap">
                    <div class="ops-dots">${dotsHtml}</div>
                    <span class="ops-counter">1 / ${others.length}</span>
                </div>
                <button class="ops-nav-btn"
                    onclick="(function(){
                        var sl=document.getElementById('${sliderId}');
                        ...
                        sl.querySelectorAll('.ops-dot').forEach((d,i)=>d.classList.toggle('active',i===next));
                        sl.querySelector('.ops-counter').textContent=(next+1)+' / '+items.length;
                    })()">&#8250;</button>
            </div>` : '';
    ...
    createModalContent(place) {
        ...
        if (place.details?.social_networks && Object.keys(place.details.social_networks).length > 0) {
            const btns = Object.entries(place.details.social_networks).map(([key, data]) => {
                const meta = socialIconMap[key] || { icon: 'fas fa-link', color: '#718096', label: key };
                return `<a href="${data.url}" target="_blank" rel="noopener" title="${meta.label}"
                           style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:8px;
                                  ...
                                  font-size:13px;font-weight:500;border:1px solid ${meta.color}30;transition:all .2s ease;"
            ...
            <div style="padding:30px;">
                ...
                <div class="place-details">
                    <div style="margin-bottom:30px;">
                        ...
                        ${place.details?.long_description ? `
                            <p style="color:#666;line-height:1.6;font-size:16px;margin-top:15px;">${place.details.long_description}</p>` : ''}
                    </div>
                    ...
                    ${place.details?.services ? `
                        <div style="margin-bottom:30px;">
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:10px;">
                                ${(typeof place.details.services === 'string' ? JSON.parse(place.details.services) : place.details.services)
                                    .map(s => `<span style="background:#e0e7ff;color:#3730a3;padding:5px 12px;border-radius:20px;font-size:13px;">${s}</span>`).join('')}
                            </div>
                        </div>` : ''}
                ...
                <div style="display:flex;gap:15px;margin-top:30px;padding-top:25px;border-top:1px solid #e0e0e0;">
                    ...
                    <button onclick="window.mapApp.closeModal()"
                            style="flex:1;padding:14px;background:#f0f0f0;color:#333;border:none;border-radius:8px;
                                   cursor:pointer;font-size:16px;font-weight:500;display:flex;align-items:center;
    ...
    renderPlacesList() {
        ...
        this.places.forEach(place => container.appendChild(this.createPlaceElement(place)));
    }
    ...
    createPlaceElement(place) {
        ...
        div.addEventListener('mouseenter', () => {
            const md = this.markers[place.id];
            if (md?.popup) md.popup.setLatLng([place.latitude, place.longitude]).openOn(this.map);
        });
        div.addEventListener('mouseleave', () => {
            setTimeout(() => {
                const popEl = document.querySelector('.leaflet-popup');
                if (!popEl || !popEl.matches(':hover')) this.map.closePopup();
            }, 100);
        });
        return div;
    }
    ...
    locateUser() {
        if (!navigator.geolocation) { this.showNotification('La géolocalisation n\'est pas supportée', 'error'); return; }
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                if (error.code === error.PERMISSION_DENIED)    msg += 'Veuillez autoriser l\'accès à votre position.';
    ...
    updatePlacesCount() {
        ...
        if (el) el.textContent = this.places.length;
    }
    ...
    setupEventListeners() {
        /* Province dropdown → zoom only */
        const provinceFilter = document.getElementById('province-filter');
        if (provinceFilter) {
            provinceFilter.addEventListener('change', e => {
                if (e.target.value) this.zoomToProvince(e.target.value);
                else this.map.setView([56.1304, -106.3468], 4);
            });
        }

        /* Category dropdown → filter points */
        const categoryFilter = document.getElementById('category-filter');
        if (categoryFilter) {
        ...
        const regionToProvince = { all: null, qc: 'qc', qb: 'ab', on: 'on' };

        document.querySelectorAll('.destinations-vedette-v2-filter-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                /* active state */
                document.querySelectorAll('.destinations-vedette-v2-filter-btn')
                    .forEach(b => b.classList.remove('active'));
                btn.classList.add('active');

                const filter      = btn.dataset.filter;
                const provinceCode = regionToProvince[filter] ?? null;

                if (!provinceCode) {
                    /* show all */
                    this.places.forEach(p => {
                        const md = this.markers[p.id];
                        if (md) md.marker.addTo(this.map);
                    });
                    ...
                    this.places.forEach(p => {
                        const md = this.markers[p.id];
                        if (!md) return;
                        const prov = (p.province_code || p.province || '').toLowerCase();
                        if (prov.includes(provinceCode)) md.marker.addTo(this.map);
    ...
    getCategoryColor(category) {
        const found = this.getStaticCategories().find(c => c.value === category);
    ...
    capitalizeFirstLetter(str) { return str ? str.charAt(0).toUpperCase() + str.slice(1) : ''; }
    escapeHtml(text) {
        if (!text) return '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.info-card').forEach(card => {
        ...
        card.addEventListener('mouseleave', function() { this.classList.remove('pulse'); });
    });
});
...
function sendHeight() {
    ...
    if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'setHeight', iframeId: 'affichez-vos-entreprises', height }, '*');
    }
}
