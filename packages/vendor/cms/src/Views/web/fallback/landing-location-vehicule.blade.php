            ...
            <div class="container">
                ...
                <div class="blog-grid">
                    @foreach($blogCards as $post)
                        @php $postImage = $mediaUrl(data_get($post, 'featured_image') ?: data_get($post, 'image') ?: data_get($post, 'thumbnail')) ?: ($gallery[$loop->index]['thumbnail'] ?? $gallery[0]['thumbnail']); @endphp
                        <a class="blog-card" href="{{ data_get($post, 'url') ?: '#' }}"><div class="blog-img"><img src="{{ $postImage }}" alt="{{ data_get($post, 'title') }}"></div><div class="blog-body"><div class="blog-date">{{ data_get($post, 'date') ?: 'Blog' }}</div><h3 class="blog-title">{{ data_get($post, 'title') }}</h3><p class="blog-excerpt">{{ \Illuminate\Support\Str::limit(strip_tags((string) (data_get($post, 'excerpt') ?: data_get($post, 'content'))), 140) }}</p></div></a>
                    @endforeach
                </div>
            </div>
        </section>
