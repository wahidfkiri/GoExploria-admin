@php
    ...
    $heroSlides = collect($sliders ?? [])
        ...
        ->map(function ($slide) use ($devisLink) {
            $type = strtolower((string) data_get($slide, 'type', 'image')) === 'video' ? 'video' : 'image';
            $mediaUrl = data_get($slide, 'image_url')
                ?: data_get($slide, 'image_path')
                ?: data_get($slide, 'video_embed_url')
                ?: data_get($slide, 'video_url');
            ...
            return [
                ...
                'video_embed_url' => data_get($slide, 'video_embed_url') ?: data_get($slide, 'video_url'),
            ];
        })
...
/* ====================== NAVBAR ====================== */
#navbar{position:fixed;top:0;left:0;right:0;z-index:1000;background:var(--nav-bg);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border-bottom:1px solid var(--border);transition:var(--transition);}
...
/* ====================== MOBILE NAV ====================== */
.mobile-nav{display:none;position:fixed;inset:0;z-index:999;background:var(--dark-2);padding:6rem 2rem 2rem;overflow-y:auto;}
