@php
    ...
    $cmsProductsTitle = isset($etablissement) && data_get($etablissement, 'id') && function_exists('get_ecommerce_section_title')
        ? get_ecommerce_section_title($etablissement->id)
        : ($cmsProductsTitle ?? 'Nos Produits disponible');
    $cmsProductsTitle = trim((string) $cmsProductsTitle) !== '' ? $cmsProductsTitle : 'Nos Produits disponible';
...
@if($cmsLandingProducts->isNotEmpty())
    <section class="cms-live-products" id="{{ $cmsProductsSectionId }}">
        <style>
            .cms-live-products{margin:18px 0;padding:clamp(22px,3vw,38px);border-radius:28px;background:radial-gradient(circle at top left,var(--cms-accent-glow,rgba(230,82,22,.16)),transparent 34%),linear-gradient(135deg,rgba(255,255,255,.96),rgba(248,250,252,.92));border:1px solid rgba(15,23,42,.08);box-shadow:0 22px 55px rgba(15,23,42,.10);overflow:hidden}
            .cms-live-products-head{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:22px}
            .cms-live-products-kicker{display:inline-flex;align-items:center;gap:8px;margin-bottom:8px;color:var(--cms-accent,#e65216);font-size:.78rem;font-weight:900;letter-spacing:.12em;text-transform:uppercase}
            .cms-live-products-title{margin:0;color:var(--cms-ink,#101827);font-size:clamp(1.75rem,3vw,2.7rem);line-height:1.05;font-weight:950}
            .cms-live-products-subtitle{max-width:640px;margin:8px 0 0;color:var(--cms-muted,#64748b);font-size:.98rem;line-height:1.65}
            ...
            .cms-live-product-card:hover{transform:translateY(-5px);border-color:var(--cms-accent-border,rgba(230,82,22,.35));box-shadow:0 24px 55px rgba(15,23,42,.14)}
            ...
            .cms-live-product-price{position:absolute;left:14px;bottom:14px;display:inline-flex;align-items:center;gap:6px;padding:9px 13px;border-radius:999px;color:#fff;background:var(--cms-ink,#101827);box-shadow:0 12px 28px rgba(15,23,42,.24);font-size:.88rem;font-weight:900}
            .cms-live-product-body{display:flex;flex:1;flex-direction:column;padding:18px}
            .cms-live-product-chip{width:fit-content;max-width:100%;margin-bottom:10px;padding:5px 10px;border-radius:999px;color:var(--cms-accent,#e65216);background:var(--cms-accent-soft,rgba(230,82,22,.10));font-size:.72rem;font-weight:900;letter-spacing:.04em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
            .cms-live-product-name{margin:0 0 8px;color:var(--cms-ink,#0f172a);font-size:1.12rem;line-height:1.24;font-weight:900}
            .cms-live-product-desc{margin:0 0 16px;color:var(--cms-muted,#64748b);font-size:.91rem;line-height:1.58}
            .cms-live-product-foot{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:auto;padding-top:14px;border-top:1px solid rgba(15,23,42,.07);color:var(--cms-ink,#0f172a);font-size:.84rem;font-weight:800}
            .cms-live-product-order{display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:40px;border:0;border-radius:999px;padding:10px 14px;color:#fff;background:var(--cms-accent,#e65216);font-size:.82rem;font-weight:950;cursor:pointer;white-space:nowrap;transition:transform .2s ease,background .2s ease}
            .cms-live-product-order:hover{transform:translateY(-1px);background:var(--cms-accent-strong,#c2410c)}
